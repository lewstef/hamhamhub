...
export function UsersTable({ userList }: UsersTableProps) {
  ...
  const totalPages = showPagination ? Math.ceil(filteredUsers.length / PAGE_SIZE) : 1;
  const safePage = Math.min(page, Math.max(1, totalPages));
  const pagedUsers = showPagination
        ...
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <Card className="relative w-full max-w-md shadow-2xl">
            ...
            <CardHeader className="px-6 pt-6 pb-4 border-b border-border">
              <CardTitle className="text-lg font-bold">Register New User</CardTitle>
      ...
      {showDeleteConfirm && deleteTargetId && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <Card className="relative w-full max-w-sm shadow-2xl">
            ...
            <CardHeader className="px-6 pt-6 pb-4 border-b border-border">
              <CardTitle className="text-lg font-bold">Delete User</CardTitle>
              <CardDescription className="text-xs mt-1">
                  ...
                  pagedUsers.map((u) => (
                    <tr key={u.id} className="hover:bg-muted/40 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-right space-x-1">
                        <Link
                          href={`/backoffice/users/edit/${u.id}`}
