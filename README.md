...
- **Modularized Course Form & Sub-Service Architecture (`src/components/course-form/`)**:
  ...
  - **Platform-Wide Diacritic-Insensitive Search**: Integrated unified diacritic normalization across **all search inputs and filters in the platform** via `normalizeSearchText` in `src/lib/utils.ts` (converting `ă`, `â` -> `a`, `î` -> `i`, `ș`, `ş` -> `s`, `ț`, `ţ` -> `t`, `é` -> `e`, `ü` -> `u`, etc.). Applied across:
    - **Users Directory** (`UsersTable` `/backoffice/users`)
    - **Employees Directory** (`EmployeesTable` `/backoffice/employees`)
    - **Organizations & Categories Directory** (`OrganizationsTable` `/backoffice/organizations`)
    - **Service Types Directory** (`ServiceTypesTable` `/backoffice/service-types`)
    - **Services Master Overview & Detail Listings** (`DashboardServicesList` & `DashboardServiceDetail`)
    - **Backoffice & Dashboard Navigation Search** (`BackofficeSidebar` & `DashboardSidebar`)
    - **Organization Edit Dropdowns** (`EditOrganizationForm` country, bank, county, and locality search)
    - **Shared Searchable Dropdowns** (`CustomSelect`)
...
```bash
# Run all tests (753 tests across 62 test files)
npm run test
...
| Config, Types & utilities | `types/course` (100% coverage with `DOG_GROOMING_WEIGHT_TIERS` & `formatWeightRanges`), `config/service-types`, `config/dog-training`, `config/romanian-territory`, `config/romanian-cartiere`, `lib/utils` (including `cn` & `normalizeSearchText`), `lib/email`, `lib/validation`, `lib/validations` (`auth`, `organizations`, `courses`, `employees`, `users`, `system`, `validations.test.ts`), `testing/mock-factories` |
| Hooks | `use-mobile` |
